...
namespace FrbaOfertas.CrearOferta
{
    public partial class Form1 : Form
    {
        ...
        public Form1(string userId,string rolId)
        {
            ...
            if (util.getVal("SELECT id FROM tp.Rol WHERE nombre = 'Proveedor'").ToString()==rolId)
            {
                lblProveedor.Hide();
                CUIT.Hide();

                proveedorId = util.getVal("SELECT proveedor FROM tp.Usuario WHERE id=" + userId).ToString();
            }
        ...
        private void btnPublicar_Click(object sender, EventArgs e)
        {
            if (this.sonCamposValidos())
            {
                if (proveedorId == null)
                {
                    var table=util.tableQuery("SELECT id FROM tp.Proveedor WHERE CUIT= @cu",
                                            "@cu",CUIT.Text);
                ...
                while (true)
                {
                    ...
                    if (util.tableQuery("SELECT 1 FROM tp.Oferta WHERE codigo = '" + codigoStr+ "'").Rows.Count == 0)
                        //improbable que haya colisiones pero no esta de mas probar.
                        break;

                }

                var command = new SqlCommand("INSERT INTO tp.Oferta (codigo,descripcion, cantidad, fecha, fecha_Venc, " +
